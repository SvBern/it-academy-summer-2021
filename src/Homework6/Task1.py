...
*************************************************************************************
 ...
 1. Расчет вознаграждения банка по обслуживанию банковских счетов/услуг разных типов
...
class CreditAccountOwners(AccountOwners):
    ...
    def calculation_fees(self):
        return round((((self.amount_of_credit / 100) * self.interest_rate) / 365) * 30)
...
class IndividualWithCard(DebitWithCashbackOwners, SMSMixin):
    """Класс для физического лица, открывшего депозит с кешбеком с выпуском карты(для примера)"""
    ...
    def withdraw_money(self):
        print(f'{self.name} снял {self.sum} руб. с карты к счету {self.account_num}')
        SMSMixin.send_sms()
...
def calculation_fees(account_owners):
    """Функция формирует список клиетов с номерами счетов, типами счетов и платой за обслуживание"""
    total = 0
    for account_owner in account_owners:
        print(f'Вознаграждение банка за обслуживание типа *{account_owner.type_account}* '
...
debit_with_cashback_owner = DebitWithCashbackOwners('Сергей Сергеев', '102938', 10, 2)
